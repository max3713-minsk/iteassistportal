...
import { LogResultView } from "@/components/logs/LogResultView";
...
export default function MonitoringLogs() {
    ...
    const logTasks = useMemo(() => allTasks.filter((t) => isLogAnalysisTask(t.title, t.description)), [allTasks]);
    const eqList = useMemo(() => {
        const out = equipmentList.map((e) => {
    ...
    const scanMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (r?.error)
                toast({ title: "Сканирование с ошибкой", description: r.error, variant: "destructive" });
            else
                toast({ title: "Сканирование завершено", description: `Найдено: ${r?.found ?? 0}` });
            qc.invalidateQueries({ queryKey: ["equipment-log-files", selectedEq] });
        },
        onError: (e) => toast({ title: "Ошибка сканирования", description: e.message, variant: "destructive" }),
    });
    async function analyzeFromSftp(fileRow) {
        if (!selectedEq)
            return;
        ...
        try {
            ...
            if (fetched.error)
                throw fetched.error;
            const { text, filename, size_bytes } = (fetched.data ?? {});
            if (!text)
            ...
            if (insErr)
                throw insErr;
            await supabase.from("equipment_log_files")
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-[340px_1fr] gap-4">
      ...
      <Card className="self-start">
        <CardHeader className="pb-3">
          ...
          <div className="relative mt-2">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск по имени / модели" className="pl-7 h-8 text-sm"/>
          </div>
        ...
        <CardContent className="px-2">
          <ScrollArea className="h-[520px]">
            {eqList.length === 0 ? (<p className="text-sm text-muted-foreground px-3 py-6 text-center">
                Нет оборудования. Снимите фильтр или добавьте устройства.
              </p>) : (<ul className="space-y-1">
                {eqList.map((e) => {
                const active = e.id === selectedEq;
                return (<li key={e.id}>
                      <button type="button" onClick={() => { setSelectedEq(e.id); setSelectedTask(e.tasks[0]?.id ?? logTasks[0]?.id ?? null); }} className={`w-full text-left rounded-md px-3 py-2 transition-colors ${active ? "bg-primary/10 border border-primary/40" : "hover:bg-muted/60"}`}>
                        <p className="text-sm font-medium truncate">{e.name}</p>
                        <p className="text-xs text-muted-foreground truncate">{e.model ?? "—"} · {e.category ?? "—"}</p>
                        <div className="mt-1 flex flex-wrap gap-1">
                          {e.tasks.length === 0 ? (<Badge variant="outline" className="text-[10px] text-muted-foreground">нет лог-задач в регламенте</Badge>) : (<>
                            {e.tasks.slice(0, 2).map((t) => (<Badge key={t.id} variant="outline" className="text-[10px]">{t.title}</Badge>))}
      ...
      <div className="space-y-4">
        {!selectedEqObj ? (<Card>
                ...
                <div className="grid gap-2 sm:grid-cols-[1fr_auto] items-end">
                  <div>
                    ...
                    <Select value={selectedTask ?? ""} onValueChange={setSelectedTask}>
                      ...
                      <SelectContent>
                        {taskOptions.map((t) => (<SelectItem key={t.id} value={t.id}>{t.title}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
            ...
            <Card>
              ...
              <CardContent>
                {sftpFiles.length === 0 ? (<div className="py-8 text-center text-xs text-muted-foreground">
                    Реестр пуст. Настройте хранилище логов на карточке оборудования и запустите сканирование.
                  </div>) : (<div className="space-y-1">
                    {sftpFiles.map((f) => (<div key={f.id} className="flex items-center gap-2 rounded-md border px-3 py-2 text-sm">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => analyzeFromSftp(f)} disabled={analyzingId === f.id}>
                          {analyzingId === f.id
            ...
            <Card>
              ...
              <CardContent>
                {logs.length === 0 ? (<div className="py-10 text-center text-sm text-muted-foreground">
                    ...
                    return (<AccordionItem key={l.id} value={l.id}>
        ...
        <LogAnalysisDialog open={dlgOpen} onOpenChange={(v) => { setDlgOpen(v); if (!v)
        refetch(); }} equipmentId={selectedEq} protocolItemId={null} protocolId={null} equipmentName={selectedEqObj?.name} taskTitle={taskOptions.find((t) => t.id === selectedTask)?.title}/>
      </div>
    </div>);
}
